...
public final class PixPayloadFactory {
    ...
    private static Map<String, EmvNode> index(List<EmvNode> nodes) {
        return nodes.stream()
                .collect(Collectors.toMap(
                        n -> (n instanceof EmvLeaf l ? l.id() : ((EmvComposite) n).id()),
                        n -> n
                ));
    }
    ...
    private static EmvField<?> toEmvField(EmvNode node) {
        ...
        List<? extends EmvField<?>> children = comp.children().stream()
                .map(PixPayloadFactory::toEmvField)
                .toList();

        return new CompositeEmvField(
                null,
                comp.id(),
                (List<EmvField<?>>) children
        );
    }

    private static CompositeEmvField composite(
            Map<String, EmvNode> map,
            String id,
            String fieldName
    ) {
        EmvNode node = map.get(id);
    ...
    public static PixPayload fromNodes(List<EmvNode> nodes) {
        ...
        return new StaticPixPayload(
                new EmvField<>("Payload Format", "00",
                        Integer.parseInt(leafValue(map, "00")), new DigitFormatter(2, PaddingPolicy.LEFT)),

                new EmvField<>("POI Method", "01",
                        Integer.parseInt(leafValue(map, "01")), new DigitFormatter(2, PaddingPolicy.LEFT)),

                composite(map, "26", "Merchant Account"),

                new EmvField<>("MCC", "52",
                        Integer.parseInt(leafValue(map, "52")), new DigitFormatter(4, PaddingPolicy.LEFT)),

                new EmvField<>("Currency", "53",
                        Integer.parseInt(leafValue(map, "53")), new DigitFormatter(4, PaddingPolicy.LEFT)),

                map.containsKey("54")
                        ? new EmvField<>("Amount", "54",
                        new BigDecimal(leafValue(map, "54")), new AmountFormatter())
                        : null,

                new EmvField<>("Country", "58",
                        leafValue(map, "58"), new StringFormatter(2, CharsetPolicy.EMV_COMMON)),

                new EmvField<>("Merchant Name", "59",
                        leafValue(map, "59"), new StringFormatter(25, CharsetPolicy.EMV_COMMON)),

                new EmvField<>("Merchant City", "60",
                        leafValue(map, "60"), new StringFormatter(15, CharsetPolicy.EMV_COMMON)),

                map.containsKey("62") ?
                        composite(map, "62", "Additional Data") : null
        );
    }
}
